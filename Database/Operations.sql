...
--From the locations where fires have burned more than 500 acres, list if there were any tsunamis have occured there too.

SELECT *
From(SELECT substr(f_latitude, 1,2) as lat, substr(f_longitude, 1, 3) as long
    FROM Fires
...
FROM(
    ...
    WHERE substr(e_date, 1,1) LIKE S_month AND
        substr(substr(e_date, 3,3),1,2) LIKE S_day AND
        substr(e_date, 6,6) LIKE S_year ) as res1, Waves
WHERE substr(res1.S_latitude, 1, 2) LIKE substr(res1.e_latitude, 1, 2) AND
